...
async function backupCollection(db: Db, collectionName: string) {
  ...
  try {
    ...
    // Check if backup collection already exists, if so drop it
    ...
    await db.collection(backupCollectionName).insertMany(data)
    console.log(
      `Backed up ${data.length} documents from ${collectionName} to ${backupCollectionName}`
    )
...
async function migrateTenant(tenant: Tenant) {
  ...
  console.log(`Creating backups for tenant ${tenantId}...`)
