...
import { TRANSACTION_BUSINESS_USER_AGE_PLATFORM_RISK_FACTOR } from '../business-user-age-platform'
...
dynamoDbSetupHook()
describe('Business User Age Platform TRANSACTION Risk Factor', () => {
  const tenantId = getTestTenantId()
  test('V8 result should be equivalent to V2 result', async () => {
    const riskFactor = {
      ...TEST_TRANSACTION_RISK_PARAMETERS[0],
      isDerived: true,
      parameter: 'businessCreatedTimestamp' as RiskFactorParameter,
  ...
  test('V8 result should handle null businessCreatedTimestamp', async () => {
    const riskFactor = {
      ...TEST_TRANSACTION_RISK_PARAMETERS[0],
      isDerived: true,
      parameter: 'businessCreatedTimestamp' as RiskFactorParameter,
