...
import { TRANSACTION_FOREIGN_DESTINATION_BUSINESS_COUNTRY_RISK_FACTOR } from '../foreign-destination-business-country '
...
import { RiskScoreValueLevel } from '@/@types/openapi-internal/all'
import { RiskParameterLevelKeyValue } from '@/@types/openapi-internal/RiskParameterLevelKeyValue'
dynamoDbSetupHook()
describe('Foreign Destination Country business TRANSACTION Risk Factor', () => {
  const tenantId = getTestTenantId()
  test('V8 result should be equivalent to V2 result foreign', async () => {
    const riskFactor = {
      ...TEST_TRANSACTION_RISK_PARAMETERS[0],
      parameter:
        'domesticOrForeignDestinationCountryBusiness' as RiskFactorParameter,
      isDerived: true,
  ...
  test('V8 result should handle null transaction country for foreign destination country business ', async () => {
    const riskFactor = {
      ...TEST_TRANSACTION_RISK_PARAMETERS[0],
      parameter:
        'domesticOrForeignDestinationCountryBusiness' as RiskFactorParameter,
  ...
  test('V8 result should handle null user country for foreign destination country business ', async () => {
    const riskFactor = {
      ...TEST_TRANSACTION_RISK_PARAMETERS[0],
      parameter:
        'domesticOrForeignDestinationCountryBusiness' as RiskFactorParameter,
    ...
    const transaction: TransactionLogicData = {
      ...
      receiverUser: getTestBusiness({
        legalEntity: {
          ...getTestBusiness().legalEntity,
          companyRegistrationDetails: undefined,
        },
      }),
      senderUser: getTestBusiness(),
      type: 'TRANSACTION',
  ...
  test('V8 result should handle null user country and transaction country for foreign destination country business ', async () => {
    const riskFactor = {
      ...TEST_TRANSACTION_RISK_PARAMETERS[0],
      parameter:
        'domesticOrForeignDestinationCountryBusiness' as RiskFactorParameter,
