...
describe('Origin Payment Method TRANSACTION Risk Factor', () => {
  ...
  test('Basic case', async () => {
    const riskFactor = {
      ...
      parameter: 'type' as RiskFactorParameter,
    ...
    const v8RiskFactor: RiskFactor = {
      ...
      riskLevelLogic: (
        getRiskFactorLogicByKeyAndType('type', 'TRANSACTION') ?? (() => [])
      )({
  ...
  test('V8 result should handle null transaction type', async () => {
    const riskFactor = {
      ...TEST_TRANSACTION_RISK_PARAMETERS[0],
      parameter: 'type' as RiskFactorParameter,
      riskLevelAssignmentValues: [
