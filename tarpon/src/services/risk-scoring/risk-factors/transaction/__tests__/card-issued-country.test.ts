...
import { TRANSACTION_CARD_ISSUED_COUNTRY_RISK_FACTOR } from '../card-issued-country'
...
dynamoDbSetupHook()
describe('Card Issued Country TRANSACTION Risk Factor', () => {
  const tenantId = getTestTenantId()
  test('V8 result should be equivalent to V2 result', async () => {
    const riskFactor = {
      ...TEST_TRANSACTION_RISK_PARAMETERS[1],
      isDerived: true,
      parameter: 'cardIssuedCountry' as RiskFactorParameter,
    ...
    const v8RiskFactor: RiskFactor = {
      ...
      riskLevelLogic: (
        getRiskFactorLogicByKeyAndType('cardIssuedCountry', 'TRANSACTION') ??
        (() => [])
      )({
  ...
  test('V8 result should handle null cardIssuedCountry', async () => {
    const riskFactor = {
      ...TEST_TRANSACTION_RISK_PARAMETERS[1],
      isDerived: true,
      parameter: 'cardIssuedCountry' as RiskFactorParameter,
