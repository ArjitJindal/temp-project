...
withFeaturesToggled(['ALERT_SLA', 'PNB'], ['CLICKHOUSE_ENABLED'], () => {
  describe('test sla service', () => {
      ...
      describe('basic SLA calculation logic test', () => {
        ...
        test('should return the Ok status and time for open alert with 10 day SLA', async () => {
          ...
          expect(result).toEqual({
            ...
            timeToBreach: expect.any(Number),
          })
        })
    ...
    describe.skip('test calculation of SLA status for all non closed alerts', () => {
