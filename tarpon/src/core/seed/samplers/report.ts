...
export class ReportSampler extends BaseSampler<Promise<Report>> {
  ...
  private async sampleFincenReport(
    ...
    const key1 = `${this.tenantId}/reports/${reportId}-${creationTimestamp}.xml`
    const key2 = `${this.tenantId}/reports/${reportId}-${
      creationTimestamp + 1
    }.xml`
    await this.uploadXMLFile(
      key1,
    ...
    return {
      ...
      revisions: [
        {
          output: `s3:document:${key1}`,
          createdAt: 1691431951412,
        },
        {
          output: `s3:document:${key2}`,
