...
export const deleteXMLFileFromS3 = async (tenantId: string) => {
  const s3Client = new S3Client({
    region: process.env.AWS_REGION,
  })
  const bucket = process.env.DOCUMENT_BUCKET
  const prefix = `${tenantId}/reports`
  if (!bucket) {
    return
  }
  try {
...
export class ReportSampler extends BaseSampler<Promise<Report>> {
  private tenantId: string
  private s3Client: S3Client
  private uploadQueue: { key: string; type: 'kenya' | 'fincen' }[] = []
  ...
  private async uploadXMLFile(Key: string, fileContent: string) {
    if (envIsNot('local')) {
  ...
  protected async generateSample(
    ...
    return await this.sampleFincenReport(
      reportId,
      caseId,
      caseUserId,
      parentReportId,
  ...
  private async sampleKenyaReport(
    reportId: string,
    caseId: string,
    caseUserId: string
  ): Promise<Report & { _id: string }> {
    ...
    const key = `${this.tenantId}/report-${reportId}-${creationTimestamp}.xml`
    await this.uploadXMLFile(key, sampleKenyaReport)
    return {
      _id: reportId,
      id: reportId,
      name: caseId,
      ...
      revisions: [
        {
          ...
          createdAt: this.rng.randomTimestamp(1000 * 60 * 60 * 24 * 30),
        },
      ],
      caseUserId,
    }
  ...
  private async sampleFincenReport(
    reportId: string,
    caseId: string,
    caseUserId: string,
    parentReportId?: string,
    ...
    const key = `${this.tenantId}/reports/${reportId}-${creationTimestamp}.xml`
    await this.uploadXMLFile(key, sampleFincenReport)
    return {
      _id: reportId,
      id: reportId,
      name: caseId,
