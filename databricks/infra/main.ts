...
import {awsAccounts} from "@flagright/lib/constants";
...
const awsAccountId = awsAccounts[stage]
...
class MyStack extends TerraformStack {
  ...
  constructor(scope: Construct, name: string) {
    ...
    const databricksSecret = new aws.dataAwsSecretsmanagerSecret.DataAwsSecretsmanagerSecret(this, 'databricks-secret', {
      name: "databricksClientSecret",
    })
    const databricksSecretVersion = new aws.dataAwsSecretsmanagerSecretVersion.DataAwsSecretsmanagerSecretVersion(this, 'databricks-secret-version', {
  ...
  private workspace({
    ...
    const bronzeCatalog = new databricks.catalog.Catalog(
      this,
      'bronze-catalog',
      {
        provider: workspaceProvider,
    ...
    new databricks.grants.Grants(this, 'bronze-grants-env', {
      provider: workspaceProvider,
      catalog: bronzeCatalog.name,
      grant: [
        {
    ...
    const spark =
      new databricks.dataDatabricksSparkVersion.DataDatabricksSparkVersion(
        this,
        'spark-version',
        {
          provider: workspaceProvider,
    ...
    new databricks.globalInitScript.GlobalInitScript(this, 'env_script', {
      provider: workspaceProvider,
      name: 'env',
      contentBase64: Fn.base64encode(
        Fn.templatefile(`${__dirname}/init.sh.tftpl`, {
              ...
              condition: [
                {
                  ...
                  values: [
                    `arn:aws:iam::${databricksAccountId}:role/${prefix}-uc-access`,
                  ],
                },
              ],
            },
...
const app = new App()
new MyStack(app, `databricks-stack-${env}`)
