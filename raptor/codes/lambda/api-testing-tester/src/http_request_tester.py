...
class Boto3Loader(object):
    ...
    def get_client(self, service):
        if self.profile is None:
            ...
            return boto3.Session(profile_name=self.profile).client(service, region_name='eu-central-1')


    def get_resource(self, service):
        if self.profile is None:
            return boto3.Session().resource(service, region_name='eu-central-1')
        else:
            return boto3.Session(profile_name=self.profile).resource(service, region_name='eu-central-1')
