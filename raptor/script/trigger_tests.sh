...
aws sns publish --topic-arn arn:aws:sns:"$REGION":"$ACCOUNT":"$PROJECT_PREFIX"-TestTrigger-Topic --profile $PROFILE_NAME --message file://script/input_data.json --region $REGION