...
import { generateTransactionRequestBody } from '../../support/utils';
describe('Case Creation test', () => {
  ...
  it('should create a case when the rule is hit', () => {
    ...
    cy.wait('@createdRule').then((interception) => {
      ...
      const requestBody = generateTransactionRequestBody(transactionId);
      ...
      cy.contains(requestBody.originUserId).should('exist');
