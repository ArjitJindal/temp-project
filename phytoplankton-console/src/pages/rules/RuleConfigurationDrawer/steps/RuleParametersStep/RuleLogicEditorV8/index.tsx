...
import { RuleLogicConfig } from '@/apis';
...
export function RuleLogicEditorV8(props: Props) {
  ...
  const config = useMemo(() => {
    if (isSuccess(ruleLogicConfig.data)) {
      if (state === null) {
        const { variables, functions, operators } = ruleLogicConfig.data?.value ?? {};
        const config = getConfig(variables ?? [], functions ?? [], operators ?? []);
        setState({
    ...
    return getConfig([], [], []);
  }, [props.jsonLogic, ruleLogicConfig.data, state]);
