...
function ExpandedRowRenderer(props: Props) {
  ...
  const tabItems = useAlertTabs({
    ...
    sanctionsSearchIdFilter: selectedItem?.hitContext?.paymentMethodId
      ? undefined
      : alert.ruleId === 'R-169'
      ? undefined
      ...
      tabBarExtraContent={
        <Select
          ...
          options={sanctionDetails.map((detailsItem) => ({
            ...
            value: detailsItem.hitContext?.paymentMethodId ?? detailsItem.searchId,
          }))}
          onChange={(value) => {
            const selectedItem = sanctionDetails.find((item) => {
              if (item.hitContext?.paymentMethodId === value) {
                return true;
              }
              if (item.searchId === value && !item.hitContext?.paymentMethodId) {
                return true;
              }
              return false;
            });
