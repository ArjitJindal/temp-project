...
export interface Props<Item extends object, Params extends object = CommonParams> {
  ...
  rowEditing?: {
    ...
    onCancel?: (rowKey: string, item: Item) => void | Promise<void>;
  };
  rowDeleting?: {
    onDelete: (rowKey: string, item: Item) => void | Promise<void>;
  };
}
    ...
    meta: {
      ...
      getRowApi: (row: TanTable.Row<TableRow<Item>>) => {
        ...
        return {
          isEditing,
          isCreateRow: false,
          isBusy: Boolean(editBusy[id] || deleteBusy[id]),
          ...
          cancelEdit: async () => {
            ...
            try {
              const result = cancel(id, original);
          ...
          delete: rowDeleting?.onDelete
            ? async () => {
                ...
                try {
                  await rowDeleting.onDelete(id, original);
                } finally {
                  setDeleteBusy((prev) => {
                    const { [id]: __, ...rest } = prev;
                    return rest as Record<string, boolean>;
